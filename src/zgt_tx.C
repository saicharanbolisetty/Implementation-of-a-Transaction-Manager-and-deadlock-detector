...
void *readtx(void *arg) {
	...
	zgt_tx *tptr = get_tx(node->tid);
	if (tptr != NULL) {
		tptr->print_tm();
		if (tptr->status == TR_ABORT) {       //Transaction is aborted
			fprintf(logfile, "T%d\t%c \tReadTx\n", node->tid, node->Txtype); // Writing log record
			fflush(logfile);
			zgt_v(0);       // Release transaction manager
			tptr->print_tm();
		} else if (tptr->status == TR_ACTIVE) {       //Transaction is active
			zgt_v(0);       // Release transaction manager
			int lockobtained = tptr->set_lock(node->tid, tptr->sgno, node->obno,
					node->count, 'S');       // Lock availability

			if (lockobtained == 0) {			//Lock is obtain for transaction
				tptr->perform_readWrite(tptr->tid, node->obno, 'S');
			}
		} else {
...
void *writetx(void *arg) { //do the operations for writing; similar to readTx
	...
	if (tptr != NULL) {
		...
		if (tptr->status == TR_ABORT) {       //transaction is aborted
			...
			fflush(logfile);
			zgt_v(0);       // Release transaction manager
		} else if (tptr->status == TR_ACTIVE) {
			zgt_v(0);       // Release transaction manager
			int lockfound = tptr->set_lock(node->tid, tptr->sgno, node->obno,
					node->count, 'X');
			if (lockfound == 0) {
				tptr->perform_readWrite(tptr->tid, node->obno, 'X');
			}
...
void *committx(void *arg) {
	...
	zgt_p(0); // locking the transaction manager
	txptr->status = TR_END; // Ending transaction
...
void *do_commit_abort(long t, char status) {
	...
	if (txptr != NULL) {
		txptr->free_locks(); //Releasing all locks
		zgt_v(t); //Releasing the transaction
		txptr->end_tx();  //Removing the transaction node
		if (status == TR_ABORT) {
		...
		printf(" There is no transaction with this tid %d", t);
...
int zgt_tx::remove_tx() {
	...
	for (txptr = ZGT_Sh->lastr; txptr != NULL; txptr = txptr->nextr) { // scan through list
		if (txptr->tid == this->tid) {		// if required node is found
			lastr1->nextr = txptr->nextr;	// update nextr value; done
			//delete this;
			return (0);
		} else
			lastr1 = txptr->nextr;			// else update previous value
	}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count,
		char lockmode1) {
	...
	zgt_tx* txNode = get_tx(tid1); // search for the transaction
	zgt_hlink* objNode = ZGT_Ht->find(sgno1, obno1); // search for the object

	if (objNode == NULL)  // if object not present
			{
		ZGT_Ht->add(txNode, sgno1, obno1, lockmode1); //adding to the hash table
		lockGranted = true;
		zgt_v(0);         // Release transaction manager
	} else  // else object present
	{
		if (objNode->tid == tid1) // if current transaction holds the lock => S lock granted
				{

			lockGranted = true;
			zgt_v(0);       // Release transaction manager
			txNode->status = TR_ACTIVE;
			txNode->semno = -1;

			zgt_v(0);         // Release transaction manager

		} else // some other transaction holds the lock
		{
			if (lockmode1 == 'S') {
				if (objNode->tid != tid1) //Currently locked by another transaction
						{
					// zgt_tx* txNode1 = get_tx(objNode -> tid); // search for the transaction object

					if (get_tx(objNode->tid)->Txtype == 'R'
							&& txNode->Txtype == 'R')  // both are read only
									{
						//update head pointer of transaction to point to the object node
						//              objNode -> nextp = txNode -> head;
						if (txNode->semno != -1) //WAIT QUEUE NOT EMPTY-YES
								{
							while (objNode != NULL) {
								lockGranted = true;
								if (txNode->head == NULL) //At Start tx with no object
									txNode->head = objNode;
								else {
									zgt_hlink* elementInLink = txNode->head;
									while (elementInLink->nextp != NULL) {
										elementInLink = elementInLink->nextp;
									}
									elementInLink->nextp = objNode;
								}
							}
						} else {
							//WAIT QUEUE NOT EMPTY-NO

							txNode->status = TR_WAIT;
							txNode->obno = obno1;
							txNode->lockmode = lockmode1;

							wait_for* w = new wait_for();
							w->wait_for::deadlock();
							if (w->wait_for::deadlock()) {
								do_commit_abort(txNode->tid, TR_ABORT);
								// txNode->status = TR_ABORT;
								zgt_v(0);  // releasing the transaction manager
								finish_operation(txNode->tid);
								pthread_exit (NULL);     // thread exit
							} else {
								txNode->setTx_semno(objNode->tid, objNode->tid); //Set semaphore on transaction
								printf("Tx %d is waiting on:T%d\n", txNode->tid,
										objNode->tid);
								// wait_for* w=new wait_for();
								// w->wait_for::deadlock();
								zgt_v(0);       // Release transaction manager
								zgt_p(objNode->tid); // wait on the transaction which has a lock

							}
						}

					} else {
						//lock cannot obtain for R/W Transaction
						txNode->status = TR_WAIT;
						txNode->obno = obno1;
						txNode->lockmode = lockmode1;
						wait_for* w = new wait_for();
				...
				if (objNode->tid != tid1) {
					...
					zgt_v(0);       // Release transaction manager
					txNode->status = TR_ACTIVE;
					txNode->semno = -1;       //TRANSACTION QUEUE IS EMPTY
...
int zgt_tx::free_locks() {
	...
	for (temp; temp != NULL; temp = temp->nextp) {	// SCAN Tx obj list
		...
		if (ZGT_Ht->remove(this, 1, (long) temp->obno) == 1) {
			printf(
					":::ERROR:node with tid:%d and onjno:%d was not found for deleting",
					this->tid, temp->obno);		// Release from hash table
			fflush (stdout);
		} else {
#ifdef TX_DEBUG
...
zgt_hlink *zgt_tx::others_lock(zgt_hlink *hnodep, long sgno1, long obno1) {
	...
	{
		if ((ep->obno == obno1) && (ep->sgno == sgno1)
				&& (ep->tid != this->tid))
...
void zgt_tx::perform_readWrite(long tid, long obno, char lockmode) {
	...
	if (lockmode == 'X')		// write op
		...
		fprintf(logfile,
				...
				this->tid, obno, ZGT_Sh->objarray[obno]->value,
				ZGT_Sh->optime[tid], this->status);
		fflush(logfile);
